...
const i18n = {
  en: {
    ...
    sectionLabelContact:  "Get In Touch",
  ...
  ar: {
    ...
    sectionLabelContact:  "دعنا نتواصل",
    sectionTitleContact:  "تواصل معي",
...
function applyLanguage() {
  ...
  const navKeys  = ["navProjects", "navAbout", "navContact"];
  ...
  const sections = [
    { label: "sectionLabelWork",    title: "sectionTitleProjects" },
    { label: "sectionLabelAbout",   title: "sectionTitleStory"    },
    { label: "sectionLabelContact", title: "sectionTitleContact"  },
  ];
  const labels = document.querySelectorAll(".section-label");
  const titles = document.querySelectorAll(".section-title");
  sections.forEach((s, i) => {
    if (labels[i]) labels[i].textContent = t(s.label);
    if (titles[i]) titles[i].textContent = t(s.title);
  });

  // Contact items
  const contactH3s = document.querySelectorAll(".contact-item h3");
  if (contactH3s[0]) contactH3s[0].textContent = t("contactEmail");
  if (contactH3s[1]) contactH3s[1].textContent = t("contactWhatsApp");
  ...
  // Re-render projects (updates visitLink text)
...
function applyAboutText(meta) {
  ...
  if (!at) return;
  const isAr = currentLang === "ar";
  const p1 = isAr ? (meta.aboutP1Ar || meta.aboutP1 || t("aboutP1")) : (meta.aboutP1En || meta.aboutP1 || t("aboutP1"));
  const p2 = isAr ? (meta.aboutP2Ar || meta.aboutP2 || t("aboutP2")) : (meta.aboutP2En || meta.aboutP2 || t("aboutP2"));
  if (meta.aboutP1En || meta.aboutP1Ar || meta.aboutP1) {
    at.dataset.custom = "1";
...
// ─────────────────────────────────────────────────────────────────────────────
//  SCROLL ARROWS (horizontal projects track)
// ─────────────────────────────────────────────────────────────────────────────
function initScrollArrows() {
  const track = document.getElementById("projectsGrid");
  if (!track) return;

  // Calculate one card width + gap for smooth scroll
  function cardWidth() {
    const card = track.querySelector(".project-card");
    if (!card) return 340;
    return card.offsetWidth + 32; // 32 = gap 2rem
  }
...
// ─────────────────────────────────────────────────────────────────────────────
//  PROJECTS RENDER — default logo image, no external URLs
// ─────────────────────────────────────────────────────────────────────────────
  ...
  grid.innerHTML = list.map((p, i) => `
    <div class="project-card" data-id="${p.id}">
      <div class="project-card-thumb">
        <img src="majd-removebg-preview.png" alt="${p.title}" draggable="false">
      </div>
...
function openProject(id) {
  ...
  // Always use the default logo in modal too
  const imgDiv = document.getElementById("projectModalImg");
  imgDiv.innerHTML = `<img class="project-modal-img" src="majd-removebg-preview.png" alt="${p.title}" style="object-fit:contain;background:#111;">`;
...
async function openAdmin() {
  ...
  try {
    ...
    const sy = document.getElementById("statYearsInput");
    const sc = document.getElementById("statClientsInput");
    if (sy) sy.value = meta.yearsExp || "";
    if (sc) sc.value = meta.clients  || "";
  } catch (_) {}
}
...
// ─────────────────────────────────────────────────────────────────────────────
//  ADD / DELETE PROJECT — no image upload, logo is always default
// ─────────────────────────────────────────────────────────────────────────────
...
function renderAdminProjects() {
  ...
  if (!list) return;
  if (!projects.length) {
    list.innerHTML = `<p style="color:#444;font-size:13px;">No projects added yet.</p>`;
    return;
  }
